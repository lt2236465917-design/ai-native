...
These rules apply to the entire `ai-native` repository. This repository contains one reusable Codex Skill; it is not an implementation of Anthropic's SDLC and must not be presented as an Anthropic product.
...
Run the complete local validation before committing behavior or template changes:
...
- Personal local discovery uses `$HOME/.agents/skills/ai-native` in the current OpenAI documentation.
